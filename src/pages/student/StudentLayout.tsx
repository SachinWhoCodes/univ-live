...
const sidebarItems = [
  ...
  { icon: MessageSquare, label: "Messages", href: "/student/messages", badge: 1 },
...
export default function StudentLayout() {
  ...
  const isActive = (href: string) => {
    if (href === "/student/tests") {
      return location.pathname.startsWith("/student/tests");
    }
          ...
          <div className="p-4 border-b border-border">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-pastel-mint">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-foreground truncate">{studentProfile.name}</p>
                <p className="text-xs text-muted-foreground truncate">{studentProfile.batch}</p>
              </div>
            </div>
          </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-80 rounded-xl">
                ...
                <DropdownMenuItem className="flex flex-col items-start gap-1 cursor-pointer">
                  <span className="font-medium">AI Review Complete</span>
                  <span className="text-xs text-muted-foreground">
                    Your CUET General Test review is ready
                  </span>
                </DropdownMenuItem>
                <DropdownMenuItem className="flex flex-col items-start gap-1 cursor-pointer">
                  <span className="font-medium">New Test Available</span>
                  <span className="text-xs text-muted-foreground">
                    CUET Physics Mock Test added to your library
                  </span>
                </DropdownMenuItem>
                <DropdownMenuItem className="flex flex-col items-start gap-1 cursor-pointer">
                  <span className="font-medium">Rank Update</span>
                  <span className="text-xs text-muted-foreground">
                    You moved up 2 positions in the leaderboard!
                  </span>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-2 px-2 rounded-xl">
                  ...
                  <span className="hidden sm:block text-sm font-medium">{studentProfile.name.split(' ')[0]}</span>
                </Button>
              </DropdownMenuTrigger>
